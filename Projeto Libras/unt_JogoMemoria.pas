...
var
  frm_JogoMemoria: Tfrm_JogoMemoria;

implementation

{$R *.DFM}

procedure Tfrm_JogoMemoria.ReiniciaJogo;
begin
  B2 := nil;
...
procedure Tfrm_JogoMemoria.CronometroTimer(Sender: TObject);
begin
  PerdePontos(1);  // perde um ponto por segundo
end;

procedure Tfrm_JogoMemoria.ClickBotao(Sender: TObject);
begin
...
procedure Tfrm_JogoMemoria.PerdePontos(N: integer);
begin
  Pontos := Pontos - N;
...
procedure Tfrm_JogoMemoria.GanhaPontos(N: integer);
begin
  Pontos  := Pontos  + N;
  Acertos := Acertos + 1;
  LabelPontos.Caption := 'Pontos: '+IntToStr(Pontos);
  if Acertos = 14 then
    FimDeJogo('Parabéns! Ganhou!!! ');
end;

procedure Tfrm_JogoMemoria.FimDeJogo(mensagem: string);
begin
...
procedure Tfrm_JogoMemoria.btnReiniciaClick(Sender: TObject);
begin
  ReiniciaJogo;
end;

procedure Tfrm_JogoMemoria.LimpaBotao(Botao: TSpeedButton);
begin
  Botao.Spacing := 200;  // cobre a figura com espaços
  Botao.Down := False;
end;

procedure Tfrm_JogoMemoria.FormCreate(Sender: TObject);
begin
  ReiniciaJogo;
end;

procedure Tfrm_JogoMemoria.Embaralha;
var
...
begin
  ...
  for i := 1 to 14 do
    for j := 1 to 2 do begin
      p := Random(13)+1;  // gera um número entre 1..6
...
procedure Tfrm_JogoMemoria.btMenuClick(Sender: TObject);
begin
    frm_JogoMemoria.Close;
end;

procedure Tfrm_JogoMemoria.btnCartaSerena1Click(Sender: TObject);
begin
 if B2 = nil then // vazio (ver ReiniciaJogo)
    B2 := B1;  // o botão anterior
  B1 := TSpeedButton(Sender);  // O botão clicado
